...
def main():
    ...
    parser_2.add_argument('--ckpt', type=str)
    parser_2.add_argument('--id', type=str)
    ...
    dm.prepare_data()
    dm.setup()
    ckpt_file = "model_checkpoints/" + args_2.ckpt
    arg_quality_model = ArgQualityModel.load_from_checkpoint(ckpt_file)

    wandb.init(project="ArgumentQuality", name=args.model_name, resume=True, id=args_2.id)
